...
function hasLanguage(value: string, language: string) {
  return value.toLowerCase().split(",").map((item) => item.trim()).includes(language);
}

function languageLabel(movie: LiveMovie) {
  const knownLanguages = ["Telugu", "Hindi", "Tamil", "Malayalam", "Kannada"];
  const match = knownLanguages.find((language) => hasLanguage(movie.language, language.toLowerCase()));
...
export default function MoviesPage() {
  ...
  const liveMovies = useMemo(() => {
    ...
    return (data?.liveMovies || []).filter((movie) => {
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_330px] lg:items-end">
            <div>
              ...
              <p className="mt-5 max-w-2xl leading-7 text-slate-600">
                Explore curated Telugu, Hindi, Tamil, Malayalam, and Kannada films. Live records use OMDb and IMDb data. Announced titles use clearly labelled verified records.
              </p>
            </div>
            ...
            <div className="rounded-3xl bg-[#0f2742] p-6 text-white shadow-xl">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-300">
                Ratings, votes, posters, trailers, and release dates appear only when a reliable source provides them.
              </p>
            </div>
          </div>
        </section>
        ...
        {loading && (
          <section className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div key={item} className="animate-pulse overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        {error && (
          <section className="mt-8 rounded-3xl border border-red-200 bg-red-50 p-7">
            <p className="text-sm font-black text-red-700">Movie catalogue could not load.</p>
          ...
          <>
            <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
              ...
              <p className="text-xs font-bold text-slate-500">
                Fetched at {data?.fetchedAt ? new Date(data.fetchedAt).toLocaleString("en-IN") : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}
              </p>
            </div>
                        ...
                        <div className="relative h-72 overflow-hidden bg-[#0f2742]">
                          {movie.posterUrl ? (
                            ...
                            <div className="flex h-full items-end bg-[radial-gradient(circle_at_top_right,#3d8bb8,transparent_38%),linear-gradient(145deg,#0f2742,#1d5277)] p-6 text-white">
                              <p className="text-4xl font-black leading-none">{movie.title}</p>
                            </div>
                          )}
                          ...
                          <div className="absolute bottom-5 left-5 right-5 text-white">
                            <p className="text-xs font-black tracking-[0.16em] text-white/70">
                              {languageLabel(movie)} Ãƒâ€šÃ‚Â· {movie.year}
                            </p>
                            <p className="mt-2 text-3xl font-black leading-none">{movie.title}</p>
                          </div>
                        </div>

                        <div className="p-6">
                          <p className="text-sm font-black text-[#087ba8]">{movie.genre}</p>
                          <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600">
                            {movie.plot === "N/A" ? "Plot information is not available from the source." : movie.plot}
                          </p>

                          <div className="mt-5 grid gap-3 rounded-2xl bg-[#f8fafc] p-4">
                            <div className="flex justify-between gap-4 text-sm">
                              <span className="font-bold text-slate-500">Release</span>
                              <span className="text-right font-black">{formatDate(movie.released)}</span>
                            </div>
                            <div className="flex justify-between gap-4 text-sm">
                              <span className="font-bold text-slate-500">Director</span>
                              <span className="text-right font-black">{movie.director}</span>
                            </div>
                            <div className="flex justify-between gap-4 text-sm">
                              <span className="font-bold text-slate-500">Cast</span>
                              <span className="text-right font-black">{movie.actors}</span>
                            </div>
                          </div>

                          <div className="mt-5 flex items-center justify-between gap-4 rounded-2xl border border-[#d6e6f5] bg-[#e9f1fa] p-4">
                            <div>
                              <p className="text-xs font-black uppercase tracking-[0.12em] text-[#087ba8]">IMDb rating</p>
                              <p className="mt-1 text-2xl font-black text-[#0f2742]">
                                {movie.imdbRating === "N/A" ? "Not available" : `${movie.imdbRating} / 10`}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="text-xs font-black uppercase tracking-[0.12em] text-[#087ba8]">Votes</p>
                              <p className="mt-1 text-sm font-black text-[#0f2742]">
                                {movie.imdbVotes === "N/A" ? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â" : movie.imdbVotes}
                              </p>
                            </div>
                          </div>

                          <div className="mt-5 flex items-center justify-between gap-3 text-xs">
                            <span className="font-bold text-slate-500">Source: {movie.source}</span>
                            <a
              ...
              <section className="mt-12">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Publicly confirmed project information only. No unverified rating, poster, trailer, or release data.
                    </p>
                  </div>
                </div>
                    ...
                    >
                      <div className="relative flex h-72 flex-col justify-between overflow-hidden bg-[radial-gradient(circle_at_top_right,#a8dff7,transparent_30%),radial-gradient(circle_at_bottom_left,#f7d99d,transparent_28%),linear-gradient(145deg,#0f2742,#245e85)] p-6 text-white">
                        ...
                        <div className="relative">
                          ...
                          <p className="mt-3 text-4xl font-black leading-[0.92]">{movie.title}</p>
                          <p className="mt-4 text-sm font-bold text-white/75">{movie.status}</p>
                        </div>
                      </div>
                      ...
                      <div className="p-6">
                        <div className="rounded-2xl bg-[#f8fafc] p-4">
                          ...
                          <div className="mt-3 flex justify-between gap-4 text-sm">
                            <span className="font-bold text-slate-500">Director</span>
                            <span className="text-right font-black">{movie.director || "Not publicly confirmed"}</span>
                          </div>
                          <div className="mt-3 flex justify-between gap-4 text-sm">
                            <span className="font-bold text-slate-500">Release</span>
                            <span className="text-right font-black">{movie.releaseNote}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                          ...
                          <p className="mt-2 text-sm leading-6 text-amber-800">
                            Rating, vote count, poster, trailer, and detailed plot are not shown until a reliable source provides them.
                          </p>
                        </div>
                        ...
                        <div className="mt-5 flex items-center justify-between gap-3 text-xs">
                          ...
                          <a
                            ...
                            className="font-black text-[#00ABE4]"
                          >
                            {movie.sourceName} ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢
                          </a>
                        </div>
                      </div>
                    </article>
            ...
            <section className="mt-12 rounded-3xl border border-[#d6e6f5] bg-[#e9f1fa] p-6 shadow-sm">
              ...
              <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-600">
                This catalogue supports movie discovery and research. A title appearing here does not mean it is available for investment, funding, revenue sharing, or any financial activity on FilmTrade.
              </p>
            </section>
          </>
        )}
